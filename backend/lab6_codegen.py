#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""实验6：源程序 -> 词法 -> 语法/语义 -> 四元式（Web API 内存运行）。"""
...
_LAB6_DIR = Path(__file__).resolve().parent.parent.parent / "实验6"
if str(_LAB6_DIR) not in sys.path:
    sys.path.insert(0, str(_LAB6_DIR))
...
DEFAULT_GRAMMAR_PATH = _LAB6_DIR / "grammar_input.txt"
...
def run_compile(source_code: str) -> Dict[str, object]:
    """对源程序执行词法、语法、语义分析与四元式生成，返回各阶段文本结果。"""
    src = source_code or ""
    if not src.strip():
    ...
    if not gtext:
        return {"ok": False, "errors": ["未找到实验6文法文件 grammar_input.txt。"]}
